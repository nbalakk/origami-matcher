...
(function (root, factory) {
  ...
  eq("столбец b2b → НОВАЯ",Sheet.guessGoalCol(H,"357428649"),3);
  eq("столбец еком → Новая",Sheet.guessGoalCol(H,"3000601598"),6);
  eq("столбец b2c → нет",Sheet.guessGoalCol(H,"357428736"),-1);
  eq("бюджет → Применяем",Sheet.guessBudgetCol(["Id","Бюджет","Новый бюджет по показателю","Новый бюджет Применяем"]),3);
  eq("бюджет → «не меняем»",Sheet.guessBudgetCol(["Id","Название","Бюджет (не меняем)"]),2);
